...
public sealed class TwitchCustomMessageHandlerTests : TestBase
{
    ...
    public async Task MatchingMessageShouldNotSendWhenCanSendReturnsFalseAsync()
    {
        ...
        bool handled = await this._twitchCustomMessageHandler.HandleMessageAsync(message: IncomingMessage, this.CancellationToken());
    ...
    public async Task NonMatchingMessageShouldNotSendAsync()
    {
        ...
        bool handled = await this._twitchCustomMessageHandler.HandleMessageAsync(message: misMatchMessage, this.CancellationToken());
