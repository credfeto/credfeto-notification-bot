...
internal static partial class TwitchChatLoggingExtensions
{
    ...
    public static void SendingMessage(this ILogger<TwitchChat> logger, in Streamer streamer, TwitchAuthenticationChat viewer, string message)
    {
        if (logger.IsEnabled(LogLevel.Information))
        {
            logger.SendingMessage(streamer: streamer, Viewer.FromString(viewer.UserName), message: message);
        }
    }
