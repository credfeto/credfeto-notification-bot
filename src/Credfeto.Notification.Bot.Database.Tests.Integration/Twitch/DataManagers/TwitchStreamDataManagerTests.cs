...
public sealed class TwitchStreamDataManagerTests : DatabaseIntegrationTestBase
{
    ...
    public async Task UpdateFollowerMilestoneAsync()
    {
        ...
        bool isFirstHit = await this._twitchStreamDataManager.UpdateFollowerMilestoneAsync(streamer: streamerName, followerCount: 10, cancellationToken: CancellationToken.None);
        Assert.True(condition: isFirstHit, userMessage: "Should be first hit");

        isFirstHit = await this._twitchStreamDataManager.UpdateFollowerMilestoneAsync(streamer: streamerName, followerCount: 10, cancellationToken: CancellationToken.None);
    ...
    public async Task RecordNewFollowerAsync()
    {
        ...
        int follows = await this._twitchStreamDataManager.RecordNewFollowerAsync(streamer: streamerName, username: followerName, cancellationToken: CancellationToken.None);
        Assert.Equal(expected: 1, actual: follows);

        follows = await this._twitchStreamDataManager.RecordNewFollowerAsync(streamer: streamerName, username: followerName, cancellationToken: CancellationToken.None);
