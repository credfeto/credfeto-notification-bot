...
public sealed class TwitchStreamOfflineTests : TestBase
{
    ...
    {
        TwitchStreamOffline offline = new(
        ...
        Assert.Equal(expected: TEST_STREAMER, actual: offline.Streamer);
        Assert.Equal(expected: TEST_TITLE, actual: offline.Title);
        Assert.Equal(expected: TEST_GAME_NAME, actual: offline.GameName);
        Assert.Equal(expected: TEST_STARTED_AT, actual: offline.StartedAt);
    }
}
